...
trait RedisRepository[T] {
   ...
   * @return A ZIO effect that completes with Unit or fails with RedisError.
   */
   ...
   * @return A ZIO effect that completes with an Option[T] (Some(entry) if found, None otherwise)
   * or fails with RedisError.
   */
  ...
  /**
   * Retrieves all entries matching the keyPrefix.
   * Note: For very large datasets, consider paginated approaches instead of KEYS *.
   * @return A ZIO effect that completes with a List[T] of all found and decoded entries
   * or fails with RedisError if any step fails (e.g., Redis communication, decoding).
   */
      ...
      entries <- ZIO.foreach(allFullKeys.toList) { fullKey =>
        redis.get(fullKey)
          ...
          .flatMap {
            ...
            case None =>
              ZIO.fail(RedisError.ProtocolError(s"Value for key '$fullKey' not found by GET, though key was listed by KEYS. Potential data consistency issue or race condition."))
          }
      }
    } yield entries
}
